...
class EditNewsModal extends Component {
  handleSubmit = async (values, action) => {
    ...
    const { toggleEditNewsModal, realtimeData, newsEdit } = this.props
    try {
      setSubmitting(true)
      await newsService.editNewNews(newsEdit.id, {
        title: values.title,
  ...
  render () {
    const { isShowEDitNewsModal, toggleEditNewsModal, newsEdit } = this.props
    return (
      <Modal isOpen={isShowEDitNewsModal} toggle={toggleEditNewsModal}>
        <div className=''>
          ...
          <Formik
            initialValues={{
              ...
              status: newsEdit.status,
              newsType: newsEdit.newsType
            }}
